...
class InceptionBlock(nn.Module):
    ...
    def __init__(
        ...
        # Branch 2: 1×1 → 3×3 (bottleneck reduces channels before expensive 3x3)
        self.branch2 = nn.Sequential(
            conv_bn_relu(in_channels, b2_reduce, kernel=1),
            conv_bn_relu(b2_reduce, b2_out, kernel=3, padding=1),
        )

        # Branch 3: 1×1 → 5×5 (bottleneck reduces channels before expensive 5x5)
        self.branch3 = nn.Sequential(
            conv_bn_relu(in_channels, b3_reduce, kernel=1),
            conv_bn_relu(b3_reduce, b3_out, kernel=5, padding=2),
        )

        # Branch 4: MaxPool → 1×1 (captures pooled features at same spatial size)
